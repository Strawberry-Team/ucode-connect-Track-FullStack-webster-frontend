...
import { SearchIcon, ChevronDownIcon, ChevronUpIcon, FunnelPlus, Square, RectangleHorizontal, RectangleVertical, X } from 'lucide-react';
...
interface SampleAssetsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddToCanvas: (type: ShapeType | "text" | "custom-image", pos: { x: number; y: number }, text?: string, settings?: Partial<ElementData>, onAdded?: (elementId: string) => void) => void;
}
    ...
    try {
      ...
      img.onload = () => {
        ...
        if (shouldSetAsBackground) {
          ...
          // Then fit the image to cover the entire canvas
          setTimeout(() => {
            setImageAsBackground(imageElement.id);
        ...
        }, shouldSetAsBackground ? 300 : 100); // Shorter wait since we don't use addRenderableObject for background
            ...
            <div className="mb-2">
              ...
              <div className="space-y-0 flex items-center justify-between mb-0">
                <div className="flex flex-wrap gap-1">
                  {['button', 'icon', 'mockup', 'emoji', 'sticker', 'label', 'interface', 'postcard', 'social media', 'ui element', 'mobile'].map((term) => (
                    <button
                      key={term}
        ...
        {activeTab === 'sample-backgrounds' && (
          <div className="flex flex-col p-6 pb-0 flex-1 overflow-hidden">
            ...
            <div className="mb-2">
              ...
              <div className="space-y-0 flex items-center justify-between mb-0">
                <div className="flex flex-wrap gap-1">
                  {['simple', 'minimalist', 'abstract', 'background', 'texture', 'gradient', 'blur', 'aesthetic', 'pastel', 'colorful', 'neutral'].map((term) => (
                    <button
                      key={term}
          ...
          {(selectedPixabayImageId || selectedUnsplashImageId) && (
            <div className="flex items-center space-x-3">
              <Checkbox
                ...
                onCheckedChange={activeTab === 'sample-images' ? handlePixabayBackgroundCheckboxChange : handleUnsplashBackgroundCheckboxChange}
